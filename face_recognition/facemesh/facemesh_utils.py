...
def denormalize_detections(detections, scale, pad):
    ...
    """
    detections[:, 0] = (detections[:, 0] * 128 - pad[0]) * scale[0]
    detections[:, 1] = (detections[:, 1] * 128 - pad[2]) * scale[1]
    detections[:, 2] = (detections[:, 2] * 128 - pad[0]) * scale[0]
    detections[:, 3] = (detections[:, 3] * 128 - pad[2]) * scale[1]

    detections[:, 4::2] = (detections[:, 4::2] * 128 - pad[2]) * scale[1]
    detections[:, 5::2] = (detections[:, 5::2] * 128 - pad[0]) * scale[0]
    return detections
