...
def raw_output_to_detections(raw_box, raw_score, anchors):
    """The output of the neural network is an array of shape (b, 2944, 18)
    containing the bounding box regressor predictions, as well as an array
    of shape (b, 2944, 1) with the classification confidences.
...
def denormalize_detections(detections, scale, pad):
    ...
    """
    detections[:, 0] = (detections[:, 0] * 256 - pad[0]) * scale[0]
    detections[:, 1] = (detections[:, 1] * 256 - pad[2]) * scale[1]
    detections[:, 2] = (detections[:, 2] * 256 - pad[0]) * scale[0]
    detections[:, 3] = (detections[:, 3] * 256 - pad[2]) * scale[1]

    detections[:, 4::2] = (detections[:, 4::2] * 256 - pad[2]) * scale[1]
    detections[:, 5::2] = (detections[:, 5::2] * 256 - pad[0]) * scale[0]
    return detections
...
def detector_postprocess(preds_ailia, anchor_path='anchors.npy'):
    ...
    """
    raw_box = preds_ailia[0]  # (1, 2944, 18)
    raw_score = preds_ailia[1]  # (1, 2944, 1)
